...
exports.buildTemplate = function (filedir, value) {
    ...
    if (/___template___\((.*)\)/.test(tmpIndex)) {
        let render = tp.compileAdaptive(templateDir, 'm');
        let paramOut = tmpIndex.match(/___template___\((.*)\)/)[1];
        let functionBody = /function\s*.*?\{([\s\S]*)\}/;
        let templateCode = 'var _data = ' + paramOut + ';' + render.toString().match(functionBody)[1];
        //模块模版的语法树
        JsCode = tmpIndex.replace(/return\s*___template___\((.*)\)/, templateCode);
    }
    if (/___template___es6\((.*)\)/.test(tmpIndex)) {// 为 ktemplate 支持es6
        let render = tp.compileAdaptive(templateDir, 'm', true);
        let paramOut = tmpIndex.match(/___template___es6\((.*)\)/)[1];
        render = render.replace(/}$/, '').split('"es6insertposition";')[1];//todo:
        let templateCode = 'var _data = ' + paramOut + ';' + render;
        //模块模版的语法树
